...
public class CamMover_ST001 : MonoBehaviour
{
    ...
    // will be modified to be changed by display resolution
    private float camHRangeLimit = 15.5f;
    ...
    private void Start()
    {
        ...
        StartCoroutine(CoroutineExtensions.DelayedActionCall(() => { isFollow = true; }, 1.25f));
    }
    ...
    private void ChangeCamPos()
    {
        ...
        if (camHRangeLimit <= 0f && camVRangeLimit <= 0f)
        {
            return;
        }
        ...
        //(3f - 2f * Mathf.Exp(-elipseparam / 0.03f)) (1f + 2f * Mathf.Pow(elipseparam,1.5f)) * 

        float followratio = (actfollowTime > 10 * float.Epsilon) ? 1-Mathf.Clamp01(Mathf.Pow(followThreshold, Time.deltaTime / actfollowTime)) : 1;

        var diff = followratio * (CalcTargetPos() - transform.position);
    ...
    {
        float elipseparam = (playerTransform.position.x / camHRangeLimit) * (playerTransform.position.x / camHRangeLimit) + (playerTransform.position.z / camVRangeLimit) * (playerTransform.position.z / camVRangeLimit);
        ...
        var target = (1 - ratio) * playerTransform.position + ratio * ratio * PositionElipseCam();
    ...
    private Vector3 PositionElipseCam()
    {
        ...
        var newpos = new Vector3(camHRangeLimit * Mathf.Cos(angle), 0f, camVRangeLimit * Mathf.Sin(angle));
