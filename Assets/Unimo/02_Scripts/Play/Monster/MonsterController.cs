...
{
    private static float MONmaxLifeTimeSTATIC = 12f;
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.CompareTag("Player") == true)
        {
            /*if (other.TryGetComponent<PlayerStatManager>(out var player))
            {
                ...
                //other.ClosestPoint(transform.position + new Vector3(0f, 1.5f, 0f));

                hitpos.y = 0f;

                player.Hit(collideStunTime, hitpos);

                var fxPos = (isexplodeFXAtPlayer) ? (hitpos + other.transform.position) / 2f + 1.5f * Vector3.up : hitpos + 1.5f * Vector3.up;

                var obj = Instantiate(explodeFX, fxPos, Quaternion.identity);
    ...
    public void ActivateEnemy()
    {
        ...
        {
            indicatorCtrl.ActivateIndicator();
        }
    }
    ...
    public void DeactiveEnemy()
    {
        ...
        if (indicatorCtrl != null)
        {
            indicatorCtrl.DeactivateIndicator();
        }
    }
