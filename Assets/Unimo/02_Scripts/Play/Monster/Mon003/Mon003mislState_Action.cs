...
public class Mon003mislState_Action : MonsterState_Action
{
    ...
    static readonly private Vector3 AddGravitySTATIC = new(0f, -10f, 0f);
    ...
    public override void FixedUpdateAction()
    {
        ...
        controller.transform.forward = rigidBody.velocity.normalized;
    }
    ...
    {
        var projForward = controller.transform.forward;
        ...
        var diffVec = targetPos - controller.transform.position;
    ...
    private void CalculateParabola()
    {
        ...
        float speed = flightDist * Mathf.Sqrt((Physics.gravity+ AddGravitySTATIC).magnitude / (controller.transform.position.y * Mathf.Sin(angle) + flightDist * Mathf.Cos(angle))) /Mathf.Sqrt(2*Mathf.Sin(angle));

        rigidBody.velocity = speed * controller.transform.forward;
