...
public class Mon004State_Action : MonsterState_Action
{
    ...
    {
        var toPlayerVec = controller.playerTransform.position - controller.transform.position;

        float btwangle = Mathf.Acos(Mathf.Clamp(controller.transform.forward.x * toPlayerVec.x + controller.transform.forward.z*toPlayerVec.z, -0.999f,0.999f));
        ...
        {
            float CCrot = (controller.transform.forward.IsCCRot(toPlayerVec)) ? 1f : -1f;

            var rotation = Quaternion.Euler(0f, CCrot * maxRotation * 180f/Mathf.PI, 0f);
